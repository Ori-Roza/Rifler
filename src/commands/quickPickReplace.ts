...
export async function quickPickReplaceCommand(ctx: CommandContext): Promise<void> {
  ...
  quickPick.title = 'Rifler QuickPick Replace';
  ...
  quickPick.onDidAccept(async () => {
    ...
    const action = await promptReplaceAction();
    ...
    if (action === 'replaceOne') {
      await replaceOne(selected.result.uri, selected.result.line, selected.result.character, selected.result.length, replaceText);
    } else {
      await replaceAll(query, replaceText, 'project', options, undefined, undefined, async () => {});
    }
  });
...
async function promptReplaceAction(): Promise<ReplaceAction> {
  const selection = await vscode.window.showQuickPick(
    [
      { label: 'Replace One', description: 'Replace only the selected match' },
      { label: 'Replace All', description: 'Replace all matches in project' },
      { label: 'Cancel', description: 'Abort replace' }
    ],
    { placeHolder: 'Choose replace action' }
  );

  if (!selection || selection.label === 'Cancel') return 'cancel';
  if (selection.label === 'Replace One') return 'replaceOne';
  return 'replaceAll';
}
