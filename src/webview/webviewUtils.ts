...
export function getWebviewHtml(webview: vscode.Webview, extensionUri: vscode.Uri): string {
  ...
  const undoIconUri = webview.asWebviewUri(
    vscode.Uri.joinPath(extensionUri, 'assets', 'undo_arrow_bottom_left.svg')
  );

  const bodyHtml = (cachedBodyHtml || '').replace(/__UNDO_ICON__/g, undoIconUri.toString());
  
  // Platform-specific class for CSS targeting (Issue: Windows spacing fix)
  const isWindows = process.platform === 'win32';
  const platformClass = isWindows ? 'windows' : '';
